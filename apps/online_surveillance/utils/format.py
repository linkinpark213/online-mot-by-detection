...
def snapshot_to_base64(tracker: Tracker, identifier: int) -> str:
    ...
    Returns:
        A bytearray. The base64 encoding of a tracker state array with shape (N, 267).
...
def snapshot_to_mot(tracker, current_time, confirmed_only=False, detected_only=True):
    data = '{:f}, -1, -1, -1, -1, -1, -1, -1, -1, -1\n'.format(current_time)
    for tracklet in tracker.tracklets_active:
        if (tracklet.is_confirmed() or not confirmed_only) and (tracklet.is_detected() or not detected_only):
            box = tracklet.last_detection.box
            score = tracklet.last_detection.score
            data += '{:f}, {:d}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:d}, -1, -1\n'.format(current_time,
                                                                                                tracklet.id,
                                                                                                box[0],
                                                                                                box[1],
                                                                                                box[2] - box[0],
